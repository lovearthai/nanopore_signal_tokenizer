...
class NanoporeVQModel(nn.Module):
    ...
    def __init__(self, codebook_size=8192, codebook_dim= 256, commitment_weight=1.0,orthogonal_reg_weight=1.0,codebook_diversity_loss_weight=1.0):
        super().__init__()
        ...
        #   kernel=5, stride=1, padding=2 → 输出长度 = T
        #   通道: 16 → 32
        encoder_layers.append(nn.Conv1d(64, 128, kernel_size=5, stride=1, padding=2, bias=True))
        ...
        encoder_layers.append(nn.Conv1d(128, self.latent_dim, kernel_size=25, stride=5, padding=12, bias=True))
