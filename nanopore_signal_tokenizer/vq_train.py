...
def vq_train(
    ...
    if rank == 0:
        ...
        with open(loss_csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if do_evaluate:
                writer.writerow(['epoch', 'recon_loss', 'commit_loss', 'total_loss', 'codebook_usage'])
            else:
                writer.writerow(['epoch', 'recon_loss', 'commit_loss', 'total_loss'])
    ...
    for epoch in range(num_epochs):
        ...
        if rank == 0:
            # ✅ 准备写入 CSV 的数据
            row_data = [epoch + 1, avg_recon, avg_commit, avg_total]
